...
> 版本 0.3（階段 1 MVP 已完成並加固，見 README.md；階段 2 尚未開始）
...
│   ├── templates/              ★ Jinja2，7 個檔（**介面文字為英文**，D5）
│   └── static/
│       ├── style.css           ★
...
`pytest` 全數通過，共 **112 項**，約 2 分鐘（其中出題引擎的 SymPy 驗證是大宗；`pytest tests/test_web.py -q` 約 11 秒）。
...
> 這裡是規劃時的完整形狀。階段 1 實作的 `app/generator/base.py` 是它的精簡版：裝飾器叫 `@register(...)`（並帶 `name` / `chapter` / `difficulty_notes` 供下拉選單使用）、`Step.detail_latex` 叫 `latex`、`seed` 直接是 `Problem` 的欄位，而 `check_data` 收斂成一個可呼叫的 `residual` 欄位（`residual_is_zero()` 就是驗證閘門）。`answer_kind` 與 `assets` 要到階段 2（判分、相圖）才用得上，目前尚未加入。
...
> **實作現況（v0.3）**：`app/db/models.py` 只有 `Student` 與 `UsageLog` 兩張表，其餘都是階段 2／3 才建。與上面草案的兩處差異：`Student.display_name`（自訂暱稱）沒有實作——MVP 沒有任何地方會顯示它，先不蒐集；`datetime.utcnow` 換成帶時區的 `datetime.now(timezone.utc)`（前者在 Python 3.12 起已 deprecated）。
...
> **目前位置（v0.3）：階段 0、階段 1 已完成，階段 1 另做過一輪加固（自架資產、介面英文化、審題與提交工具）。階段 2 尚未開始，下一步是作答判定（§5）。**
