...
> 版本 0.7（階段 1 已完成；**自動評分已捨棄**，階段 2 重新定義為「補齊題型 + 提升解說品質」）
...
目標結構（★ = 已實作；無標記者為階段 2／3 的預定位置。v0.7 起 `app/grader/`
...
`pytest` 全數通過，共 **146 項**，約 2.5 分鐘。分佈：

| 檔案 | 項數 | 守的是什麼 |
|---|---|---|
| `test_generators.py` | 91 | 出題引擎（SymPy 驗證是整份測試的大宗）＋答案的顯示形式一致性（§2.9） |
| `test_web.py` | 55 | 端對端流程、**答案遮蔽（D13）**、判定是否移除乾淨（D12）、前端資產、介面語言 |

> **v0.7 的變化：271 → 146。** `test_grader.py`（99）與 `test_grader_sandbox.py`（24）
> 整份移除，保存在 tag `grading-v1`。`test_web.py` 57 → 55，是「拿掉 19 項判定測試、
> 加上 17 項新測試」的淨值——**移除一個功能不等於測試變少**，見下面第三張表。
...
> **目前位置（v0.7）：階段 0、階段 1 已完成並加固。階段 2 因 D12 重新定義為「補齊題型 + 提升解說品質」，尚未開始。**
>
> v0.4–v0.6 做的作答判定已隨 D12 移除（保存在 tag `grading-v1`），因此階段 2 的工作量從 3–3.5 PW 降到約 2.5 PW，而且**整段變成同一件事的重複**：每個新題型就是一個新檔案 + 一組 `Check` + 一份逐步解答。沒有第二個難的子系統要蓋。

#### 為什麼下一步是「題型」而不是「相圖」或「預生成」
...
### 階段 2（v0.7 重新定義）：補齊題型與提升解說品質（約 2.5 PW）— ⬜ 尚未開始

**範圍與建議順序**（由上到下就是建議的施工順序）

| # | 項目 | 估計 | 為什麼排在這裡 |
|---|---|---|---|
| 2a | **待定係數**（`undetermined.py`，含共振重數 `m` = 0/1/2 三個難度） | 0.5 PW | 課程份量最重、學生最常卡的題型，而且 §2.4(e) 的模板與逐步解答**已經寫好並實測過**，照著落地即可。它也會第一次帶進「純量的初值問題」 |
| 2b | **恰當方程**（`exact.py`，難度 3 需要積分因子） | 0.4 PW | §2.4(c) 的反向構造是全部題型裡最漂亮的一個（先寫位勢函數 $F$，恰當性自動成立），落地風險最低 |
| 2c | **逐步解答的一次整體審查**（`scripts/preview.py` 批次產樣本，老師逐題看） | 0.4 PW | 見下面「解說品質」。應該**排在 2a、2b 之後**——先讓要審的東西到齊，一次審完，不要審兩遍 |
| 2d | **一階線性系統的其餘情況**：重根（缺陷矩陣，需廣義特徵向量）、複數特徵值（實數形）、非齊次 | 0.6 PW | 難度高於前面幾項：重根要處理 $\mathbf{x} = (\mathbf{v}t + \mathbf{w})e^{\lambda t}$，複數要確保寫成實數形而非 $e^{(a+bi)t}$（D11 的顯示形式規則要跟著擴充） |
| 2e | **參數變異法**（`variation.py`，$g(x)$ 走白名單） | 0.3 PW | §2.4(f)：隨機的 $g$ 幾乎必然積不出初等形式，所以要老師從課本挑一組白名單（§7 #14 仍待決） |
| 2f | **Laplace 變換**（`laplace.py`，含步階函數與延遲項） | 0.5 PW | 步驟最多、最需要「一行對應課本一步」，因此排最後——前面 2c 審過的解說風格正好可以套用 |
| — | 相圖 SVG、離線預生成 | — | **不排入**，理由見下 |

**交付**：涵蓋課程全部範圍的題庫，每個題型都有經過人工審過的逐步解答。

**驗收標準**：每個新題型 × 每個難度隨機 30 題全部通過 `tests/test_generators.py` 的六項檢查
（殘差為 0、無特殊函數與未算完的積分、無醜分數、係數在白名單內、逐步解答非空且最後一步等於答案、
同 seed 可重現）；`GEN_TEST_SAMPLES=200` 的完整回歸也要綠。
...
- **相圖**（Matplotlib → SVG）是**新的一種東西**（新相依、新的快取問題、新的一類測試），
  而且它只服務系統題型的一小部分。在課程還有一半題型沒有的時候做它，順序是錯的。移到階段 5（可選）。
- **離線預生成**：實測一次出題約 0.1 秒，30 人同時在線完全足夠；D7 讓題目可由 seed 重現之後，
  `Problem` 表的另一個理由（歷史紀錄要能重現）也消失了。**降級為「效能真的不夠時再做」**，
  不要為了填滿路線圖而做。
...
- 相圖與方向場 SVG（Matplotlib `Agg`）+ 快取。由階段 2 移來，理由見階段 2。
...
11. **題目敘述的英文用語要不要對齊課本？** D5 已定案用英文，但用詞還沒對過課本（例如 "general solution" / "complete solution"、"integrating factor" 的引入方式）。這件事適合用 `scripts/preview.py` 一次產一批樣本來審（§2.8），而且已排進階段 2 的 2c。
22. **（v0.7 新增）逐步解答的顆粒度要訂成什麼規則？** 目前四個 generator 是 4–6 步，切法各憑感覺。建議「一步 = 課本上會單獨寫一行的一個動作」，但要在階段 2 的 2c 審過樣本之後才定得下來。捨棄判定之後這是產品的核心，值得花時間。
