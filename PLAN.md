...
> 版本 0.6（階段 1 已完成；階段 2 的**作答判定**已完成並加固，題型完整化與相圖尚未開始）
...
│   │   ├── equivalence.py      ★ 三值等價判定（D9）、常數是否線性獨立
│   │   ├── core.py             ★ 判定流程（跑在子行程裡）
│   │   ├── feedback.py         ★ 判定結果 → 英文分層回饋
│   │   └── sandbox.py          ★ 常駐 worker 與 timeout（D6、D10）；叫不起來就 fail fast（D8）
...
│   ├── grader_sampling_report.py ★ 量測判定各條路徑的比例（§5.3）
...
`pytest` 全數通過，共 **271 項**，約 3 分鐘。分佈與各自的耗時：

| 檔案 | 項數 | 耗時 | 守的是什麼 |
|---|---|---|---|
| `test_generators.py` | 91 | 約 130 秒 | 出題引擎（SymPy 驗證是整份測試的大宗）＋答案的顯示形式一致性（§2.9） |
| `test_web.py` | 57 | 約 31 秒 | 端對端流程、前端資產、介面語言、啟動自檢（D8） |
| `test_grader.py` | 99 | 約 9 秒 | 作答判定的數學正確性、三值等價判定（D9）與輸入解析 |
| `test_grader_sandbox.py` | 24 | 約 46 秒 | 子行程、timeout 的隔離（D10）、建不起來時的 fail fast 與告警（會真的開行程，所以獨立成一個檔） |

> `test_grader.py` 從 13 秒降到 9 秒是 D9 的副作用：錯答通常在第一個取樣點就被反證，
> 根本不會進到 `simplify`。
...
| 類別 | 測試 | 為什麼需要 |
|---|---|---|
| 三值判定（`test_grader.py`，8 項） | 抽樣單獨絕不足以證明「是 0」；`is_zero` 這個兩值包裝要把 `unknown` 壓成 False；`unknown` 要變成 `unverified` 而不是 correct／partial；寫成 `cosh+sinh` 的正確答案要判對；落到 `unknown` 要留 WARNING 且 log 不含學生作答 | 守 D9 的**方向性**。最關鍵的是第一項：它把「反證失敗 + 證明失敗」同時注入，逼出唯一一條可能誤判的路徑，確認它回的是 `unknown` 而不是 `zero` |
| 逾時的隔離（`test_grader_sandbox.py`，8 項） | 一個逾時的請求不得讓同時進行的正常請求失敗；逾時只少掉一個 worker、倖存的那個會被重用（用 `spawned_total` 驗，整池重建會讓計數多跳一次）；冷啟動與第一次真判定都不得吃掉判定時限；排隊逾時回 `busy` 而不是無限等待 | 守 D10。這些行為都不會讓功能測試變紅——舊版整池重建時，**每一項功能測試也都是綠的**，壞的只有「同時剛好在判定的那個人」 |
| 顯示形式（`test_generators.py`，16 項） | 每個 (題型, 難度) 的答案不得混用指數與雙曲寫法；線性系統的答案與逐步解答一律不得出現 `sinh`／`cosh`；`as_exponential` 不得動到三角函數 | 守 D11。它不會拋錯、只會讓學生困惑，正是最需要測試盯著的那種缺陷（和 v0.3 的「裸露 `e^{rx}`」是同一類） |

**v0.4 新增的判定測試（`test_grader.py`）** 分成四組，各自針對一種會出錯的方式：
...
- **判分才即時跑 SymPy**，但判分的運算量遠小於出題，且一定要包 timeout 與子行程（避免 `signal` 在 async 環境的限制）。子行程叫不起來時**不降級、直接失敗**（D8，理由見 §5.6）——沒有子行程就沒有 timeout，而那是一個誰都能觸發的阻斷服務漏洞。子行程的組織方式（為什麼不是 `ProcessPoolExecutor`）見 §5.7。
...
> **目前位置（v0.6）：階段 0、階段 1 已完成並加固；階段 2 的第一塊「作答判定」已完成並加固（§5 全面落地，含 `Attempt` 表與 "My Progress" 頁；v0.5 拔掉靜默降級，v0.6 處理掉判定的三個不穩：偽陽性、逾時的隔離、顯示形式）。階段 2 剩下的是題型完整化、相圖與離線預生成——下一步建議先補題型，因為判定的骨架已經在了，每個新題型只要多給一個 `Check` 就自動有判分。**
...
15. ~~難度 3 的系統初值題，答案會出現 `sinh`／`cosh` 混著 `exp`。~~ → v0.6 已處理（D11、§2.9）。
...
19. **`unverified` 真的發生時，要不要主動通知老師？** 目前只留一行 WARNING 並寫進 `Attempt`。若學期中真的出現，代表化簡管線遇到沒涵蓋到的寫法，而學生當下是被卡住的。要不要做成每日彙總信？先觀察一學期再說——實測語料是 0 筆，可能整學期都不會發生。
20. **判定的時限、worker 數要不要隨題型調整？** 現在是全域的 5 秒／2 個。系統題（向量、兩倍工作量）與可分離變數（非線性、隱式解要 `solve`）的成本差得不少，但目前都在時限內，沒有調整的必要。等 `Attempt.duration_ms` 累積一學期的真實資料再看分布。
21. **化簡管線要不要留一條「昂貴但完整」的後路？**（例如 `simplify(force=True)`、或對特定題型做 `dsolve` 交叉驗證。）目前 `unverified` 是死路一條——學生只能換寫法。若真的有學生反覆卡住，可以考慮讓 `unverified` 的作答進一個離線佇列，晚上用更長的時限重跑一次再回報。成本不低，先不做。
