...
> **v0.44：摺積展示頁重排（D78），殘響時間改成量出來的（D79）。測試 1295 → 1307（+12）。**
>
> - 老師：「資訊量太多且可調的參數太多，不曉得要從何調起」。新版最上面是
>   **三個按鈕與一個選單，沒有任何滑桿**：播語音 → 選一個地方、聽它的
>   脈衝響應 → 聽同一句話在那裡說出來。
> - ⛔ **舊的第二段（48 kHz，四個旋鈕）整段刪掉**；舊的第一段與第三段
>   **一個字都沒有改**，收進一個預設不展開的 `<details>`。
>   ⚠️ 老師只指定第一段要收，**第三段是我一起收的**（理由寫在 D78）。
> - ⛔ **老師最初的「車水馬龍的道路」在數學上不成立**：車聲是
>   $y = x*h + n$ 的那個 $n$，**加上去的，不是摺積出來的**。
>   用 `AskUserQuestion` 問過才動手，改成四個真的環境；老師當輪追加第五個
>   「包在棉被裡」——⚠️ **那一個是低通不是空間**，於是同一頁上同時出現了
>   摺積的兩種面貌（回彈幾次／哪些頻率活得下來）。
> - ⛔ **`measuredRt60()` 這條獨立的路當場抓到一個係數錯**：`decayTau()` 把
>   60 dB 寫成 $6\ln 10$ 而不是 $3\ln 10$，五個房間量出來整整齊齊都是參數的
>   一半。⚠️ **若畫面上報的是參數，這個錯不會有任何症狀**——見 D79。
> - 五個 $h$ 全部由程式算出來（⛔ 沒有新增任何音檔），用**寫死種子**的 PRNG，
>   並一律正規化成**單位能量**：不正規化的話換房間變成一次音量比較。
> - ⚠️ 新增 11 項 JS 數值測試，**其中一半守的不是「算得對不對」而是設計上的
>   承諾**（單位能量、確定性、棉被是唯一的低通）——那些承諾壞掉的時候
>   **聲音仍然會出來**。
...
>   `Demo.week` 從 `"Weeks 1-2"` → `"Week 2"`。
> - ⚠️ **索引頁標題的後半段會跟著換**：它來自 `curriculum.WEEKS`，不是來自
>   展示自己，所以畫面上的 `Week 1 — LTI systems and signals` 變成
>   **`Week 2 — Convolution and the impulse response`**。
> - ⚠️ **W1 因此完全沒有內容了**；`test_curriculum.py` 那個寫死的週次集合
>   如期變紅，而那正是它存在的意義。
> - ⛔ **展示的內容與題目一個字都沒有動**，v0.21 那些歷史紀錄裡的
>   「W1–W2」也沒有改——那是當時真的成立的事。
> - ⚠️ **這一輪順便證實了 D65 真的在省時間**：`select` 挑出 7 個測試檔
>   （約 180 秒），**`tests/test_generators.py` 完全沒有被挑到**；全套是 1695 秒。
>   ⛔ 「跑一次要一個多小時」那個成本**只發生在動到 `tests/data/test_deps.json`
>   的時候**——那時規則是全跑，因為地圖一動就會改變「該跑什麼」的答案。
>   新增題型的那幾輪之所以貴，是因為它們一定會動到地圖。
...
# 測試（全部 1340 項、約 20 分鐘；出題引擎的 SymPy 驗證是大宗）
pytest
...
全套 **1340 項、約 20 分鐘**，而一輪工作常常只動一兩個檔案。
