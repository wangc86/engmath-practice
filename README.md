...
- **九個題型 × 三個難度，共 27 種組合**（v0.25 新增 Fourier 的三個，見下面「題型清單」）
...
- 其餘題型（待定係數、恰當方程、參數變異、系統的重根／複數／非齊次）　※ **Laplace 已於 v0.24 完成**（工作項 2f），**Fourier 級數已於 v0.25 完成**（2B2–2B4）
- 逐步解答的整體審查與風格統一
- 相圖、離線預生成、教師後台　※ 相圖是 `Problem.assets` 的第一個使用者，而 `assets` **刻意還沒有加**（PLAN §2.2.1 的落地紀錄）
