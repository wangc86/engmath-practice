...
### 4. Convolution: the same sentence in five different places（**W2**，LTI 與摺積）

**摺積不是一個計算規則，是「系統把輸入的每一個樣本各自敲一次，再把結果疊起來」。**

> ### ⚠️ v0.44（2S10b）：這一頁重排過，下面那三段的敘述描述的是**摺疊區裡**的內容
>
> 老師的回饋是「資訊量太多且可調的參數太多，不曉得要從何調起」。
> 現在頁面最上面是**三個按鈕與一個選單，沒有任何滑桿**：
> 播原始語音 → 選一個地方、聽它的脈衝響應 → 聽同一句話在那個地方說出來。
> 五個選項是戶外街道、演奏廳、**包在棉被裡**、空房間、長隧道。
>
> ⛔ **五個都是誠實的 LTI 系統**：聽到的差別完全來自 $h$，沒有任何額外疊加。
> 這一點在頁面上有一段專門的說明——老師最初的構想裡有一個「車水馬龍的道路」，
> 而**車聲是加上去的，不是摺積出來的**（$y = x*h + n$ 的那個 $n$）。
> ⚠️ 棉被那一個與另外四個不同類：四個空間改變的是**聲音回彈幾次**，
> 棉被改變的是**哪些頻率活得下來**——同一個運算，既做得出空間感，也做得出音色。
>
> 下面原本的三段裡，**第一段與第三段整段收進一個預設不展開的摺疊區**
> （一個字都沒有改），**第二段被新的三步取代、整段刪掉**。

**（摺疊區）第一段：把一條訊號滑過另一條。** 一支平移滑桿 $n$（可拖、可打數字、也有一個
自動掃描按鈕），三張圖共用同一個 $k$ 軸：
...
~~**第二段：同一個 $h$，在四萬八千分之一秒的尺度上。**~~
⛔ **v0.44 刪掉了**：它有四個旋鈕（來源、聽哪一邊、間隔、區塊長度），
而它們回答的問題學生在還沒聽過差別之前根本還沒有產生——那正是老師說的
「不曉得要從何調起」。頁面最上面的三步取代了它，語音樣本仍然是 2S4 那一份
（這一頁**從頭到尾沒有新增過任何音檔**，五個房間的 $h$ 都是程式算出來的）。
取得回來：`git log -p -- app/static/demos/convolution.js`。

**（摺疊區）第三段：兩個性質，以及少了它們會怎樣。** 三個系統各跑同樣兩項檢查，
每一項報的是**差多少**而不是「是／否」：
...
pytest                          # 全部 1340 項，約 20 分鐘（也會順便更新相依地圖）
