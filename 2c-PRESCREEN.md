...
| 題型 | d1 | d2 | d3 | 平均 | 離中位數 |
|---|---|---|---|---|---|
| `fourier.series.full_range` | 7.8 | 9.0 | 8.8 | 8.5 | +2.1 ⚠️ |
| `fourier.series.half_range` | 6.8 | 6.4 | 6.0 | 6.4 | +0.0 |
| `ode.first_order.linear` | 5.0 | 5.0 | 5.0 | 5.0 | -1.4 |
| `ode.first_order.separable` | 5.0 | 5.0 | 5.0 | 5.0 | -1.4 |
| `ode.laplace.ivp` | 8.0 | 9.2 | 9.0 | 8.7 | +2.3 ⚠️ |
| `ode.laplace.transform` | 3.0 | 5.0 | 5.0 | 4.3 | -2.1 ⚠️ |
| `ode.second_order.homogeneous` | 4.0 | 4.0 | 4.0 | 4.0 | -2.4 ⚠️ |
| `system.linear_2x2.classification` | 4.0 | 4.0 | 4.0 | 4.0 | -2.4 ⚠️ |
| `system.linear_2x2.complex` | 8.0 | 8.0 | 10.0 | 8.7 | +2.3 ⚠️ |
| `system.linear_2x2.real_distinct` | 6.0 | 6.0 | 9.0 | 7.0 | +0.6 |
| `system.linear_2x2.repeated` | 8.0 | 8.0 | 10.0 | 8.7 | +2.3 ⚠️ |

中位數 **6.4** 步。⚠️ 標了 ⚠️ 的是離中位數 2 步以上的——**先看那幾個**。
...
⛔ **長度不是難度。** 這一節唯一能回答的是「有沒有任何一個可量的向度隨難度上升」；答案是「沒有」時**也可能是對的**（例如難度軸是共振重數，那件事量不出來——`ode.second_order.undetermined` 曾經是這樣，而它在 v0.35 被刪掉了）。
...
**敘述開頭的第一個字**：`Find` × 105、`Solve` × 30、`Use` × 15、`Classify` × 15

**幾組競爭說法各出現幾次**（兩邊都 > 0 的那幾組要拍板）：

- `general solution` × 150　vs　`complete solution` × 0
- `initial condition` × 30　vs　`initial value` × 110 ⚠️ **兩種都在用**
