...
- [7. 第一次使用](#7-第一次使用)（⚠️ v0.16：兩組共用帳號，沒有註冊頁、沒有告知頁）
...
安裝 Python  →  解壓專案  →  建立虛擬環境  →  安裝套件
             →  設一把金鑰  →  建兩組共用帳號  →  啟動 uvicorn
             →  瀏覽器開 127.0.0.1:8000  →  登入  →  出題
```

⚠️ **「建一個帳號」是 v0.15 新增的一步。** 系統不再有註冊頁，帳號由老師用一支
命令列工具建立（[7.1](#71-建立兩組共用帳號v016)）——v0.16 起是**兩組全班共用的帳號**，不是一人一組。
...
`app/static/demos/lib/` 的純函式來驗證 165 項數值斷言，另有 10 項冒煙測試把三個展示的
JS 在假 DOM 裡真的跑一遍。**沒有裝 node 的話那 175 項會
...
⚠️ **這一段在 v0.16 又改了一次。** 系統現在只有**兩組共用帳號**，沒有學號、
也沒有註冊頁（PLAN.md **D35**）。直接開 <http://127.0.0.1:8000/register> 會拿到
**404**，那是正確的行為，不是壞掉。
...
```
[已建立] 角色 class（全班共用，發給所有修課學生）
    登入名稱：class
    密　　碼：cedar-otter-flint-47

[已建立] 角色 staff（老師與助教測試用（用量不計入全班統計））
    登入名稱：staff
    密　　碼：willow-quartz-heron-83

⚠️ 上面的密碼只會出現這一次——資料庫裡只有 argon2id 雜湊。
⚠️ 這台電腦是公用的話，用完請清掉終端機的捲動紀錄。
   （忘記了也沒關係：`reset` 子指令隨時可以換一組。）
```

**沒有檔案產生。** v0.15 會寫出一份 `ACCOUNTS-PLAINTEXT-DELETE-ME-*.csv`，
那個檔案**已經不存在了**——兩組密碼用不著一份對照表，所以系統這一側再也沒有
任何含明碼的檔案。

那組 `字-字-字-數字` 的密碼是系統產生的，格式刻意做成「好念、好打」——全小寫、
數字只用 2–9，所以**沒有任何一對長得像的字元**（不會有 `l` 跟 `1`、`O` 跟 `0`
分不清的問題）。這在 v0.16 更重要了：這組密碼是要**在課堂上念給全班聽**的。

兩組帳號的差別：

| 角色 | 給誰 | 差在哪 |
|---|---|---|
| `class` | 全班共用 | 一般使用者。**測試時請用這一組**，才看得到學生看到的東西 |
| `staff` | 老師與助教 | 多一個 **Class activity** 頁；它做的事**不計入全班統計** |

其他你可能會用到的：

```powershell
python scripts\create_accounts.py list          # 看有哪些帳號（沒有密碼）
python scripts\create_accounts.py reset class   # 換一組全班的密碼
python scripts\create_accounts.py reset class --password "fourier-series-2026"
```

> ⚠️ **`init` 重跑是安全的**（已存在的會跳過，密碼不變）。要換密碼一定要用
> `reset`——因為覆寫共用帳號等於**全班同時進不來**。

### 7.2 第一次登入

1. 開 <http://127.0.0.1:8000>，會被導到登入頁。
2. 登入頁的表單下面有一段說明，**請讀一次**——它是學生會看到的東西
   （PLAN.md **D40**）：
   - 黃底框：⚠️ 這不是學校官方系統，**帳號是全班共用的，把它當成公開場合**。
   - 下面一塊列出系統實際記了什麼：全班共用一個帳號所以系統無從得知你是誰、
     只彙總「哪些題型被練、哪些展示被開」、**不存姓名／學號／IP／你打的任何東西**、
     沒有作答框也不判對錯。
3. 輸入帳號 `class` 與上一步拿到的密碼，按 **Log in** → **直接進到出題頁**。

> ⚠️ **v0.15 的「Before you start」個資告知頁沒有了**（PLAN.md **D37**）。
> 系統不再蒐集任何個人資料，所以個資法的告知義務沒有標的可以適用——
> 不是義務被放寬，是要告知的那樣東西不存在了。
> <http://127.0.0.1:8000/consent> 現在是 404。

> **想確認登入閘門真的擋得住？** 開一個「無痕視窗」（沒有 session cookie），
> 直接在網址列打 <http://127.0.0.1:8000/demos> 或
> <http://127.0.0.1:8000/activity>，都會被彈回登入頁。這不是各頁面自己在檢查，
> 是一層 middleware（`app/login_gate.py`）在擋，所以繞不過去。

### 7.3 兩件 v0.16 沒有了的事

**改密碼的頁面沒有了。** <http://127.0.0.1:8000/account/password> 是 404。
密碼是共用的——讓任何一個學生改掉它，等於把全班鎖在門外，而且不會拋任何錯誤
（PLAN.md **D35**）。要換密碼請用上面的 `reset` 子指令。

**「My Progress」沒有了**，改成 **Class activity**
（<http://127.0.0.1:8000/activity>），而且**只有 `staff` 帳號看得到**
（PLAN.md **D39**）。用 `class` 登入時：

- 導覽列上**沒有**那個連結；
- 直接打網址會拿到 **403**，訊息說這一頁只給課程的 staff 帳號。

想看那一頁，登出後改用 `staff` 的密碼登入。
...
6. ⚠️ **導覽列上沒有「看自己出過幾題」的地方，這是刻意的**（PLAN.md D39）。
   系統不知道你是誰，所以「我」練了幾題沒有答案。用 `staff` 登入才看得到
   **Class activity**（全班的彙總）。

### 7.5 三個互動展示

從導覽列進 **Demos** <http://127.0.0.1:8000/demos>，目前有三頁：
...
| **Fourier series** | <http://127.0.0.1:8000/demos/fourier/series> | 用正弦疊出方波；再把各諧波的**相位**打亂，看波形整個變、而聲音幾乎不變 |

這三頁**都會發出聲音**。請先讀下一節。

> 🎧 **第三頁請務必戴耳機、而且務必真的按下 Start sound。** 它的整個教學價值
> 在於「畫面變了、聲音沒變」這個對照，只看圖等於只拿到一半。
> 另外它是唯一一頁會在你改參數時**換掉整個波形**的展示——換的瞬間本來會有
> 「喀」一聲，程式用一段 60 ms 的音量凹陷把它蓋掉了。
> **如果你聽到明顯的爆音或喀聲，那是一個真的 bug，請記下你剛才動了哪個控制項。**
> 那段流程在開發環境裡一次都沒有真的聽過（沙箱沒有音效卡）。
...
> ⚠️ **老實說一句**：這三個展示的伺服器端該驗的都驗過了（路由、資產、頁面內容、
> 純函式層 165 項數值斷言），而且從 v0.17 起還多了 10 項「冒煙測試」——它在
> node 裡搭一個很小的假 DOM，把每一頁的 JS 真的載入、畫一次、撥動十幾個控制項，
> 因此「module 一載入就爆、畫面完全沒反應」這一類最粗的錯誤現在擋得住了。
>
> **但那不是瀏覽器。** 音訊、canvas 的實際輸出、autoplay 解鎖、worklet 載入、
> `setPeriodicWave` 到底發出什麼聲音、DPR 縮放——這些只有真的瀏覽器才跑得到的
> 部分，開發環境沒有瀏覽器，從來沒有實際執行過。這次 Windows 測試部署正好就是
> 它們的第一次驗收——遇到問題是意料之中的，不是你裝錯了。
...
- **⚠️ 檔案權限在 Windows 上是另一回事**：程式啟動時會呼叫 `os.chmod(db, 0o600)`
  ...
  多人共用的電腦上請自己確認一次。這個檔案裡有**兩組共用帳號的密碼雜湊**
  （v0.16 起沒有學號了，PLAN.md D35）。
...
- **`account`**：登入名稱（`class` / `staff`）、角色、密碼的 argon2id 雜湊、
  建立時間、最後有人登入的時間。**永遠只有兩列**，而且**沒有任何一欄指得到人**
  （v0.16／D35 拿掉了 `student_no` 與 `consent_at`）。
  **密碼本身絕對沒有存**（有一項測試會掃過整個 DB 檔案確認這件事，
  另有一項確認整個檔案裡沒有任何長得像學號的字串）。
- **`usagelog`**：哪一組帳號、什麼時候、哪個題型／展示、哪個難度、哪個 seed。
  沒有作答內容、沒有對錯、沒有滑桿位置、沒有停留時間、**沒有 IP**。
  `account_id` 只有兩個值，它唯一的用途是把 `staff` 的測試流量排除在全班統計外。
...
> ⚠️ **備份檔和正本一樣含有兩組帳號的密碼雜湊**（v0.16 起沒有學號了）。
> 真的要留存就加密（7-Zip 設密碼、或 `age`／`gpg`），不要往雲端硬碟隨手一丟。
> 測試完就刪掉最省事。
...
下次啟動會自動建一個新的空資料庫，帳號要重新建立（見 [7.1](#71-建立兩組共用帳號v016)）。
...
——那 175 項需要 node 才跑得動（`tests/test_dsp_js.py` 的 165 項數值斷言，
加上 `tests/test_demos.py` 裡 10 項把展示 JS 真的跑一遍的冒煙測試），
會 skip 並印出原因，其餘 262 項照常。要跑那 175 項就去 <https://nodejs.org/>
裝一個 LTS 版，重開 PowerShell 之後再跑一次。
...
| 缺什麼 | 為什麼要緊 |
|---|---|
| **HTTPS** | 系統處理密碼。沒有 HTTPS，登入時的帳號與密碼在網路上是明文。這是上線的硬性前提，不是加分項。 |
| **正式的 WSGI／ASGI 部署方式** | `uvicorn --reload` 是**開發伺服器**：會監看檔案變動、沒有 process 管理、掛了不會自己起來。 |
| **資料庫檔案權限** | 見 [9.9](#99-資料庫檔案相關)。Windows 上 `chmod 600` 形同虛設，而檔案裡有兩組帳號的密碼雜湊。 |
| **備份機制** | 現在只有「你記得手動複製」。 |
| **速率限制的正確性前提** | 目前的登入速率限制是**單一行程的記憶體計數器**。所以只能用**單一個 uvicorn 行程**部署；開多個 worker 會讓限制形同虛設。 |

**另外要記得的兩件事（v0.16 全部改寫了）：**

- ~~學期結束後應執行去識別化~~ → **沒有東西需要去識別化了**（PLAN.md D35）：
  資料庫裡沒有個人資料。
- ~~個資告知頁與 `UsageLog` 的欄位一字對應~~ → 告知頁沒有了（D37），
  但**看守點還在，換了理由**：`UsageLog` **不得長出任何指得到特定個人的欄位**
  （D36）。要多記任何東西之前，先看 `CLAUDE.md` 硬規則 3。
- ~~那份「學號 ↔ 初始密碼」對照表含明碼，發完就刪~~ → **那個檔案不存在了**
  （D35）：兩組密碼用不著對照表，密碼只印在終端機上一次。
- ⚠️ **新增一件**：正式部署一定會加反向代理，而 Caddy／nginx 的存取紀錄
  **預設含來源 IP**——那會讓應用層「不記 IP」（D38）的努力整個白費。
  設定見 `FREEBSD-DEPLOY.md` §5.7。
...
東西跑起來看看、特別是在真的瀏覽器裡驗收那三個展示**——那正是目前最欠缺的一塊。
...
python -m pip install -r requirements.txt
$env:SESSION_SECRET = (python -c "import secrets; print(secrets.token_hex(32))")
python scripts\create_accounts.py init          # ← v0.16：兩組共用帳號，沒有註冊頁
uvicorn app.main:app --reload
```

⚠️ `init` 會把 `class` 與 `staff` 兩組密碼**印在畫面上，而且只印這一次**
（資料庫裡只有雜湊）。抄下來。**不會產生任何檔案**——v0.15 那份
`ACCOUNTS-PLAINTEXT-DELETE-ME-*.csv` 已經沒有了。
...
然後開 <http://127.0.0.1:8000>，用帳號 `class` 與剛剛印出來的密碼登入，
**直接進到出題頁**（v0.16 起沒有個資告知頁了）。`Ctrl+C` 停止伺服器。
