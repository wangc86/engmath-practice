...
#:
#: ⛔ **老師 2026-09-08 拍板：除 2**（PLAN §7 #23 結案、D73、附錄 C.4）。
#: 也就是 $f(x) \sim \frac{a_0}{2} + \sum_{n\ge1}(\cdots)$，
#: 而 $a_0 = \frac1L\int_{-L}^{L} f\,dx$ 與 $a_n$ 共用同一條積分公式（$n \ge 0$），
#: 逐步解答第 4 步的教學說明也才撐得起來。
#: ⚠️ **v0.25–v0.39 這一格是暫定值，而暫定值與拍板值恰好相同**——
#: 所以 v0.40 那一輪**一個答案的數值都沒有變**。
#: 記在這裡是因為「沒有變」與「沒有做」看起來一樣，而它們不一樣。
#:
#: ⛔ **拍板之後仍然留著這個開關，理由與 `transform.FORWARD_EXP_SIGN`（D72）相同**：
#: 老師答的是「哪一種課本慣例」，不是「哪一行程式」。另一種慣例
#: （$a_0 + \sum$，$a_0 = \frac1{2L}\int$）確實有課本在用。
#:
...
#:
#: ⚠️ **那一項問的是耦合，不是選擇：它對兩種慣例都是綠的。**
#: 「現在選的是哪一個」由 `test_the_a0_convention_is_the_one_the_teacher_chose`
#: 釘住——它拿一個平均值為 $\frac32$ 的階梯函數，斷言 $a_0$ 是**手算的 3**
#: （不除 2 的慣例下會是 $\frac32$）。⛔ **右邊必須是手寫的常數**，
#: 從 `A0_IS_HALVED` 導出來的斷言證明不了任何事。
#:
#: 真正的成本不在程式——**題目一旦印給學生看過，他們的筆記就對不上了**。
#: 這件事本來就要在開課前決定，而 2026-09-08 決定了（課綱 W3 是 9/23）。
A0_IS_HALVED = True
