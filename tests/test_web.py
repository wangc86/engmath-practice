...
@pytest.mark.parametrize("module", [
    "app.grader",          # D12
    "app.consent_gate",    # D37（曾改名為 login_gate）
    "app.login_gate",      # D57
    "app.security",        # D57
    "app.accounts",        # D57
    "app.access_log",      # D58
    ...
    "app.db.models",       # D58
    "app.release",         # D59
    "app.release_gate",    # D59
    "app.routes.auth",     # D57
    "app.routes.release_admin",   # D59
])
def test_a_removed_module_is_really_gone(module):
    """import 不到，才叫刪掉了。

    ⚠️ 這份清單是**累積**的，不要因為「舊的那幾條顯然已經不在了」而清掉
    ——它們一條也不佔時間，而它們防的是「某次還原或合併把檔案帶回來」。
    每一個都保存在對應的 tag（`grading-v1` / `hosted-v1`）。
    """
    import importlib
...
@pytest.mark.parametrize("path, method", [
    ("/login", "GET"), ("/login", "POST"), ("/logout", "POST"),
    ("/register", "GET"), ("/register", "POST"),
    ("/consent", "GET"), ("/consent", "POST"),
    ("/account/password", "GET"), ("/account/password", "POST"),
    ("/activity", "GET"),
    ("/admin/content", "GET"), ("/admin/content", "POST"),
    ("/progress", "GET"),
    ("/openapi.json", "GET"),
])
def test_a_removed_endpoint_is_really_gone(client, path, method):
    """端點也一樣。**`/openapi.json` 在清單上是刻意的**：

    `docs_url` 與 `redoc_url` 從一開始就關了，但 FastAPI 仍然會掛一條
    `/openapi.json`（v0.28 才發現，那時它會回傳整張路由表）。
    `openapi_url=None` 關掉它之後，這一行是它不會被某次升級悄悄帶回來的
    唯一保證。
    """
    r = client.request(method, path)
    assert r.status_code == 404, f"{method} {path} 還在（{r.status_code}）"


def test_no_template_mentions_a_removed_endpoint():
    """範本裡也不准留著指向那些端點的連結。

    一個 `href="/activity"` 不會讓任何測試變紅，但它會讓使用者點到 404。
    """
    gone = ("/login", "/logout", "/register", "/consent",
            "/account/password", "/activity", "/admin/content", "/progress")
    offenders = []
    for template in TEMPLATES_DIR.rglob("*.html"):
        text = template.read_text(encoding="utf-8")
        # Jinja 註解（{# … #}）裡提到舊路徑是可以的——那是在解釋歷史。
        text = re.sub(r"\{#.*?#\}", "", text, flags=re.S)
        for path in gone:
            if f'"{path}"' in text:
                offenders.append(f"{template.name} → {path}")
